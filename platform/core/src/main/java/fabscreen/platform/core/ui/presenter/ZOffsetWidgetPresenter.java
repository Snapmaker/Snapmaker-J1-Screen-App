...
public class ZOffsetWidgetPresenter extends BasePresenter {
    @BindView(R2.id.tv_widget_z_offset_title)
    TextView mTvTitle;

    @BindView(R2.id.tv_widget_z_offset_current)
    TextView mTvValueCurrent;

    @BindView(R2.id.tv_widget_z_offset_slash)
    TextView mTvValueSlash;

    @BindView(R2.id.tv_widget_z_offset_target)
    TextView mTvValueTarget;

    @BindView(R2.id.tv_widget_z_offset_unit)
    TextView mTvValueUnit;

    @BindView(R2.id.sbg_widget_z_offset_steps)
    ...
    public ZOffsetWidgetPresenter(CompositeDisposable compositeDisposable) {
    ...
    public void connectPrintSettings() {
        float initialValue = ServiceContainer.getInstance().getService(IMachine.class).getPrintController().getOverrideZOffset();
    ...
    @OnClick(R2.id.btn_widget_z_offset_up)
    void onClickUp() {
        ...
        ServiceContainer.getInstance().getService(IMachine.class).getPrintController().setOverrideZOffset(targetValue);
    }

    @OnClick(R2.id.btn_widget_z_offset_down)
    void onClickDown() {
