...
public class LaserCameraController implements ILaserCameraController {
     ...
     * for using resolution: please check protocol definition
     *
    ...
    public Observable<Boolean> connect(String macAddress) {
        ...
        return Observable.timer(10, TimeUnit.SECONDS)
                .flatMap(tick -> {
                    Logger.d("10s gone, start inner conn");
    ...
    private Observable<Boolean> innerConnect(String macAddress) {
        return Observable.fromCallable(() -> {
            try {
                ...
                Logger.d("Connecting to BT \"%s\"", macAddress);
                mBluetoothSocket.connect();
                Logger.d("Laser camera connected takes " + (SystemClock.elapsedRealtime() - start) + "ms");
                ...
                mWriteSubscription = mWriteSerializedSubject
                        ...
                        .subscribe((p) -> mBluetoothSocket.getOutputStream().write(p), e -> {
                            LaserCameraController.this.disconnect();
                            e.printStackTrace();
                        });
                    ...
                    case BluetoothPacketBuilder.LASER_CAMERA_PHOTO_RESPONSE_EVENT_ID: {
                        ...
                        switch (operationId) {
                            ...
                            case 1:
                                ...
                                Logger.d("received packet " + mDataList.size() + " " + (SystemClock.elapsedRealtime() - mRcvTime));
                                break;
                            case 2:
                                // merge photo data
                                Logger.d("Received image done takes %d ms.", (SystemClock.elapsedRealtime() - mRcvTime));
                                byte[] pic = mergeData();
                                if (pic == null) {
                                    break;
                                }
                                ...
                                if (bitmap != null) {
                                    ...
                                    Logger.w("Decode bitmap failed.");
                                }
                                break;
                            case (byte) 0xFF:
                                // error ?
                                Logger.w("Error message from laser camera.");
                                break;
                        }
                        break;
                    }
