...
public class LaserCalibration4AxisSetOriginFragment extends BaseFragment {
    ...
    private void start() {
        int calibrationMode = ServiceContainer.getInstance().getService(IPreferences.class).getHelper().getLaser4AxisCalibrationMode();
        ...
        turnOffLaser()
                ...
                .subscribe(success -> {
                    ...
                    if (getActivity() != null) {
                        int calibrationMode = ServiceContainer.getInstance().getService(IPreferences.class).getHelper().getLaser4AxisCalibrationMode();
                        if (calibrationMode == 0) {
                            ((CalibrationLaserActivity) getActivity()).start4AxisAutoFocusStep1Fragment();
                        } else {
                            ((CalibrationLaserActivity) getActivity()).start4AxisManualFocusStep1Fragment();
                        }
                    }
                }, e -> {
                    LogHelper.log(e);
