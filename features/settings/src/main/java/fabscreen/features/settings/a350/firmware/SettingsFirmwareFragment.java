...
import fabscreen.platform.core.ui.view.ProgressButton;
...
public class SettingsFirmwareFragment extends BaseFragment {
    ...
    @BindView(R2.id.ll_firmware_new_features)
    View mViewNewFeatures;
    @BindView(R2.id.ll_firmware_bug_fixes)
    View mViewBugFixes;
    @BindView(R2.id.ll_firmware_improvements)
    View mViewImprovements;
    @BindView(R2.id.tv_firmware_change_log_version)
    TextView mTvVersion;
    @BindView(R2.id.tv_firmware_new_features)
    TextView mTvNewFeaturesDesc;
    @BindView(R2.id.tv_firmware_bug_fixes)
    TextView mTvBugFixesDesc;
    @BindView(R2.id.tv_firmware_improvements)
    TextView mTvImprovementsDesc;
    @BindView(R2.id.btn_settings_firmware_check_updates)
    Button mBtnCheckUpdate;
    @BindView(R2.id.btn_settings_firmware_download_update)
    ProgressButton mBtnDownloadUpdate;
    @BindView(R2.id.btn_settings_firmware_update)
    Button mBtnFirmwareUpdate;
    @BindView(R2.id.tv_settings_firmware_warning)
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mHttpDownloader = new HttpDownloader(requireContext().getCacheDir().getAbsolutePath());
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        ServiceContainer.getInstance().getService(IPreferences.class).getHelper().setUpdateNotification(false);
    }
    ...
    private void initView() {
        ...
        if (mNewVersionData != null) {
            ...
            String lastUpdateVersion = ServiceContainer.getInstance().getService(IPreferences.class).getHelper().getLastUpdatePackageVersion();
            if ((!mNewVersionData.version.equals(lastUpdateVersion)) && ServiceContainer.getInstance().getService(IPreferences.class).getHelper().getUpdateNotification()) {
                ServiceContainer.getInstance().getService(IPreferences.class).getHelper().setLastCheckVersion(mNewVersionData.version);
            ...
            mTvVersion.setText(ServiceContainer.getInstance().getService(IPreferences.class).getHelper().getLastUpdatePackageVersion());
        }
    ...
    @OnClick(R2.id.btn_settings_firmware_check_updates)
    void checkForUpdate() {
        ...
        apiClient.getLatestVersion()
                ...
                .subscribe(version -> {
                    ...
                    Logger.d("version %s, url %s, machine_size %d summary %s",
                    ...
                    if (version.data.new_version.version.equals(ServiceContainer.getInstance().getService(IPreferences.class).getHelper().getLastUpdatePackageVersion())) {
                        AndroidSchedulers.mainThread().scheduleDirect(this::showNewestVersionDialog, 0, TimeUnit.MILLISECONDS);
                        ServiceContainer.getInstance().getService(IPreferences.class).getHelper().setLastCheckVersion(version.data.new_version.version);
                    } else {
    ...
    @OnClick(R2.id.btn_settings_firmware_download_update)
    void downloadUpdate() {
        playNormalClickSound();
    ...
    @OnClick(R2.id.btn_settings_firmware_update)
    void onClickUpdate() {
        playNormalClickSound();
        if (getContext() != null) {
            saveVersionResponse(mNewVersionData);
            String filePath = ServiceContainer.getInstance().getService(IAppService.class).getCacheDir().getAbsolutePath() + "/update.bin";
    ...
    void factoryReset() {
        ...
        FabFullScreenDialog.create(getContext())
                ...
                .setMessage("Current version is " + BaseApplication.getInstance().getAppVersionName() + " , would you like to reset to the factory version?")
