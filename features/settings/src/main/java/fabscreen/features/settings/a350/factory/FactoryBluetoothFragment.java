...
public class FactoryBluetoothFragment extends BaseFragment {
    ...
    @BindView(R2.id.btn_factory_bluetooth_request)
    Button mBtnBluetoothRequest;
    @BindView(R2.id.btn_factory_bluetooth_camera_lighting)
    Button mBtnBluetoothLighting;

    @BindView(R2.id.tv_factory_bluetooth_status)
    TextView mTvBluetoothStatus;
    @BindView(R2.id.tv_factory_bluetooth_result)
    TextView mTvBluetoothResult;
    @BindView(R2.id.tv_factory_bluetooth_used_time)
    TextView mTvBluetoothSpeed;

    @BindView(R2.id.iv_factory_bluetooth_image)
...
//                });
    ...
    @OnClick(R2.id.btn_factory_bluetooth_request)
    void onClickRequest() {
...
//                    final int dataSize = ServiceContainer.getInstance().getService(IMachine.class).getLaserController().getLaserCameraController().getDataSize();
...
//                });
    ...
    @OnClick(R2.id.btn_factory_bluetooth_camera_lighting)
...
//                    .setSupplementaryLightStatus(status)
//                    .observeOn(AndroidSchedulers.mainThread())
//                    .as(bindToLifecycle())
//                    .subscribe(success -> {
//                        if (success) {
