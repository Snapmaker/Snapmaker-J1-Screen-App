...
//    private int mOriginJ1Toolhead;
    ...
    public Observable<Integer> getPrintStateObservable() {
        return ServiceContainer.getInstance().getService(IMachine.class).getPrintController().getPrintStateObservable();
    }

    public Observable<Integer> getMachineStatusObservable() {
        return ServiceContainer.getInstance().getService(IMachine.class).getNewPrintController().getMachineStatusObservable();
    }

    public int getMachineStatus() {
        return ServiceContainer.getInstance().getService(IMachine.class).getNewPrintController().getMachineStatus();
    }
    ...
    public void movementExtruder(int index, int moveType) {
        int printState = ServiceContainer.getInstance().getService(IMachine.class).getNewPrintController().getPrintState();
        if (!MachineOperationStatus.isPrintFilamentChangeAvailable(printState)) return;
        int movementType;
        float length;
        switch (moveType) {
            case 0:
        ...
        mFdmController.requestExtruderMovement(index, movementType, length, movementType == 0 ? 200 : 450)
                .as(bindToLifecycle())
    ...
    public void changeHeating(int index, int targetTemperature) {
        ...
        if (index == 0) {
            ...
            if (targetTemperature == 0) {
                ...
                value.setTemperature(targetTemperature);
            }
            mLeftExtruderStateSubject.onNext(value);
        } else if (index == 1) {
            value = mRightExtruderStateSubject.getValue();
        ...
        mFdmController.setExtruderTemperature(index, 0, targetTemperature)
                .as(bindToLifecycle())
                .subscribe(responseStructure -> {
