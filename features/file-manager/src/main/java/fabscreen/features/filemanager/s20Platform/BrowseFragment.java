...
import fabscreen.platform.core.ui.view.SwipeListView;
...
public class BrowseFragment extends BaseFragment {
    ...
    @BindView(R2.id.top_bar_info)
    ImageButton mIbTopBarInfo;

    // List
    @BindView(R2.id.view_browse_local_files)
    View mViewLocalFiles;
    @BindView(R2.id.lv_browse_local_files)
    SwipeListView mLvLocalFiles;
    @BindView(R2.id.view_browse_local_files_empty)
    View mViewLocalFilesEmpty;

    @BindView(R2.id.view_browse_remote_files)
    View mViewRemoteFiles;
    @BindView(R2.id.lv_browse_remote_files)
    SwipeListView mLvRemoteFiles;
    @BindView(R2.id.view_browse_remote_files_empty)
    View mViewRemoteFilesEmpty;
    @BindView(R2.id.view_browse_usb_file_unable_to_identify)
    View mViewRemoteFilesError;

    // file type bar
    @BindView(R2.id.bb_browse_file_type)
    BottomBar mBbFileTypeBar;

    // multi-select bar
    @BindView(R2.id.view_browse_multi_selected_bar)
    View mMultiSelectedBar;
    @BindView(R2.id.btn_browse_multi_delete)
    Button mBtnMultiSelectedDelete;
    @BindView(R2.id.btn_browse_multi_rename)
    Button mBtnMultiSelectedRename;

    @BindView(R2.id.iv_browse_multi_delete)
    ImageView mIvMultiSelectedDelete;
    @BindView(R2.id.iv_browse_multi_rename)
    ImageView mIvMultiSelectedRename;

    // file usage
    @BindView(R2.id.pb_browse_space_usage)
    ProgressBar mPbSpaceUsage;
    @BindView(R2.id.tv_browse_space_usage)
    ...
    private void updateLocalFilesLists() {
        ...
        mLocalViewModel.watchFilteredFiles()
                ...
                .subscribe(files -> {
                    ...
                    mCurrentViewModel = mLocalViewModel;
                    updateSpaceUsage();
                });
    }

    private void mountRemoteFiles(boolean isAttach) {
        // TODO: viewmodel dont provide mount() anymore
        if (isAttach) {
            mBbFileTypeBar.enableTab(1);
        } else {
            mBbFileTypeBar.disableTab(1);
        }
        mRemoteViewModel.listFiles().as(bindToLifecycle()).subscribe(files -> {
...
//                        });
    ...
    private void initRemoteFiles() {
        // init remote files
        mRemoteViewModel = new BrowseViewModel(getContext(), false);
    ...
    private void initLocalFiles() {
        // init local files
        mLocalViewModel = new BrowseViewModel(getContext(), true);
        mLocalViewModel.listFiles().as(bindToLifecycle()).subscribe(files -> {
    ...
    void onBack() {
        ...
        if (!mCurrentViewModel.getPartition().isRoot()) {
    ...
    @OnClick(R2.id.btn_browse_multi_delete)
    ...
    @OnClick(R2.id.btn_browse_multi_rename)
    ...
    private void updateSpaceUsage() {
        long totalSpace = mCurrentViewModel.getPartition().getTotalSpace();
        long usageSpace = mCurrentViewModel.getPartition().getUsedSpace();
