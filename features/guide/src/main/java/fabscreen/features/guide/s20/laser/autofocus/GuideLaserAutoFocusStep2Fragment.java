...
public class GuideLaserAutoFocusStep2Fragment extends BaseFragment {
    ...
    public static GuideLaserAutoFocusStep2Fragment newInstance() {
        return new GuideLaserAutoFocusStep2Fragment();
    }
    ...
    private void start() {
        ...
        ServiceContainer.getInstance().getService(IMachine.class).getMachineStatusSubjectHolder().getObservable()
                ...
                .subscribe(machineStatus -> {
                    final float offsetZ = machineStatus.originOffset.getZ();
                    Logger.d("z = " + machineStatus.currentPosition.getZ());
                    mInitialZ = machineStatus.currentPosition.getZ() - offsetZ; // calculate absolute Z
...
//                });
    ...
    void onClickNext() {
        ...
        if (mCalibrationStatusSubject.getValue() == STATUS_COMPLETE) {
            ...
            if (getActivity() != null) {
                ((GuideLaserActivity) getActivity()).startAutoFocusPickFragment();
            }
        }
    }
}
