...
public class TouchPlatformViewModel extends BaseViewModel {
    ...
    public Observable<ResponseStructure> moveXYZByStep(MoveController.Direction direction, float stepWidth) {
        mIsMovingSubject.onNext(true);
        return MoveController.getInstance()
                .moveByStep(direction, stepWidth)
                .doOnNext(responseStructure -> mIsMovingSubject.onNext(false));
    ...
    public void initToolheadPosition() {
        ...
        float initZ = ServiceContainer.getInstance().getService(IMachine.class).getMachineInfoSubjectHolder().getValue().size.getZ() * 0.5f;
    ...
    public void savePlatformZOffset() {
        Vector status = ServiceContainer.getInstance().getService(IMachine.class).getMachineStatusSubjectHolder().getValue().currentPosition;
        ...
        ServiceContainer.getInstance().getService(IPreferences.class).getHelper().setLaserPlatformZ(status.getZ() - offsetZ);
        ServiceContainer.getInstance().getService(IMachine.class).getLaserController().setFocalLength(status.getZ() - offsetZ + 10).as(bindToLifecycle()).subscribe();
    }

    public Observable<ResponseStructure> upLiftToolhead() {
        mIsMovingSubject.onNext(true);
        return MoveController.getInstance()
